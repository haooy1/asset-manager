...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchStats(), fetchReminders()]).finally(() => setLoading(false));
  }, []);
  ...
  const urgentCount = warrantyItems.filter((i) => i.daysUntilExpiry <= 7).length +
    docItems.filter((i) => i.daysUntilExpiry <= 7).length +
    expiredWarranty.length +
    expiredDocs.length;
    ...
    <div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          ...
          <div className="mt-2 text-2xl font-bold text-red-600">{stats.expiring + docItems.length}</div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Link href="/assets/new" className="rounded-xl border bg-blue-50 p-4 shadow-sm transition hover:bg-blue-100">
          <div className="text-2xl mb-1">➕</div>
          <div className="text-sm font-semibold text-blue-900">新增资产</div>
          <div className="mt-0.5 text-xs text-blue-600">录入新设备</div>
        </Link>
        <Link href="/assets/import" className="rounded-xl border bg-purple-50 p-4 shadow-sm transition hover:bg-purple-100">
          <div className="text-2xl mb-1">📥</div>
          <div className="text-sm font-semibold text-purple-900">批量导入</div>
          <div className="mt-0.5 text-xs text-purple-600">Excel导入资产</div>
        </Link>
        <Link href="/approvals" className="rounded-xl border bg-green-50 p-4 shadow-sm transition hover:bg-green-100">
          <div className="text-2xl mb-1">✅</div>
          <div className="text-sm font-semibold text-green-900">审批管理</div>
          <div className="mt-0.5 text-xs text-green-600">查看待审批</div>
        </Link>
        <Link href="/org" className="rounded-xl border bg-orange-50 p-4 shadow-sm transition hover:bg-orange-100">
          <div className="text-2xl mb-1">👥</div>
